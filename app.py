...
def extract_text_and_links_from_pdf(filepath):
    """
    Extracts both visible text and hyperlink annotations from a PDF file.
    Merges hyperlink annotations to ensure clickable profile links (e.g. LinkedIn, GitHub)
    are included in downstream analysis even when not explicitly written out in visible text.
    Includes OCR fallback for scanned/image PDFs.
    """
    extracted_text = ""
    extracted_links = []
    ...
    # OCR Fallback if text is insufficient (< 100 chars)
    ...
    # Append hyperlink annotations to extracted text (avoiding duplicates)
    if extracted_links:
        links_to_append = []
...
def calculate_resume_strength(text_lower, text_original):
    ...
    has_linkedin = bool(normalize_linkedin_url(text_lower)) or ("linkedin.com/in/" in text_lower) or ("linkedin.com" in text_lower)
...
def perform_analysis(extracted_text, job_description, filename, extraction_method):
    ...
    if len(jd_skills) == 0:
        weight_skill = 0.0
        weight_semantic = 0.4  # Reduced from 0.7
        weight_strength = 0.6  # Increased from 0.3
    elif len(jd_skills) <= 2:
        weight_skill = 0.4  # Increased from 0.3
        weight_semantic = 0.2  # Reduced from 0.5 (semantic may not be available)
        weight_strength = 0.4  # Increased from 0.2
    else:
        weight_skill = 0.6  # Increased from 0.5
        weight_semantic = 0.1  # Reduced from 0.3
        weight_strength = 0.3  # Increased from 0.2
